...
const AdminDashboard = ({ token }) => {
  ...
  const fetchReports = async (pageToFetch = 1) => {
    try {
      ...
      // const response = await fetch(`https://laisa1-pothole-patrol-backend.hf.space/api/reports?page=${pageToFetch}&limit=10`);
  ...
  const handleStatusChange = async (reportId, newStatus) => {
    try {
      ...
      // await fetch(`https://laisa1-pothole-patrol-backend.hf.space/api/reports/${reportId}/status`, {
          ...
          <tbody>
            {reports.map((report) => (
              <tr key={report._id} style={{ borderBottom: '1px solid #333' }}>
                ...
                {/* 🆕 NEW: Smart Image Rendering Logic
                <td style={{ padding: '12px' }}>
                  {report.image_url ? (
                    // 1. For ALL NEW uploads going forward (uses the exact URL from the database)
                    <img 
                    ...
                    // 2. For your OLD uploads that you manually dragged into the Cloudinary "uploads" folder
                    // ⚠️ IMPORTANT: Replace 'YOUR_CLOUD_NAME' with your actual Cloudinary name!
                    <img 
                      src={`https://res.cloudinary.com/dyid8eljr/image/upload/uploads/${report.image_filename}`} 
                      alt="Pothole Legacy" 
                      style={{ width: '60px', height: '40px', objectFit: 'cover', borderRadius: '4px', border: '1px solid #555' }} 
                    />
                  ) : (
                    // 3. If there is genuinely no image at all
                ...
                <td style={{ padding: '12px' }}>
                  {report.image_filename ? (
                    <img 
                    // src={`https://laisa1-pothole-patrol-backend.hf.space/uploads/${report.image_filename}`}
